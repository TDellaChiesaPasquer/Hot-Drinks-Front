import { StyleSheet, Modal, TextInput, View, Pressable, Text, TouchableOpacity, Dimensions, ScrollView } from "react-native";
...
export default function ListRdvScreen({ navigation }) {
	...
	const rdvHTML = rdvList.map((rdv) => {
		const otherUser = String(rdv.creator._id) === String(user.user._id) ? rdv.receiver : rdv.creator;
		...
		return (
			...
			>
				...
				<View style={styles.message}>
					<Text style={styles.username}>{otherUser.username.length > 25 ? otherUser.username.slice(0, 22) + "..." : otherUser.username}</Text>
					<Text style={styles.date}>{rdv.address.length > 35 ? rdv.address.slice(0, 32) + "..." : rdv.address}</Text>
