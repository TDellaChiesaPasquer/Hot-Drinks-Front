import { StyleSheet, View, Text, Dimensions, TouchableOpacity, ScrollView, Modal } from "react-native";
...
export default function PreferencesScreen({ navigation }) {
	...
	const defaultAgeRange = user.user && user.user.ageRange ? [Number(user.user.ageRange.slice(0, 2)), Number(user.user.ageRange.slice(3, 5))] : [20, 30];
	const [ageRange, setAgeRange] = useState(defaultAgeRange);
	const ageRangeString = ageRange[1] === 65 ? ageRange.join("-") + "+" : ageRange.join("-");
	const distanceValues = [5, 6, 8, 10, 12, 15, 20, 25, 30, 40, 50, 65, 80, 100, 125, 150, 200, 250, 300, 400, 500, 600];
	const defaultDistance = [user.user && user.user.distance ? Number(user.user.distance) : 20];
	const [distance, setDistance] = useState(defaultDistance);
	const distanceString = (distance[0] === 600 ? "500+" : String(distance[0])) + " km";
	...
	const sendInfos = async () => {
		const response = await fetch(process.env.EXPO_PUBLIC_IP + "/users/algoInfos", {
			method: "PUT",
			headers: {
...
const styles = StyleSheet.create({
	...
	iconContainer: {
		position: "absolute",
		top: 30, // Ajusté pour être sous l'image
		width: 0.18 * width,
		height: 0.18 * width,
