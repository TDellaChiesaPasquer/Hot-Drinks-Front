...
export default function ({ navigation, route }) {
  ...
  const keyboardStyleBottom = useAnimatedStyle(() => ({
    height:
      keyboard.state.value === 0
        ? 66
        : Math.max(0, keyboard.height.value - (49 + insets.bottom)) + 66,
  }));
  ...
  if (currentRdv) {
    if (currentRdv.status === "demande") {
      if (String(currentRdv.creator._id) !== String(user.user._id)) {
        currentRdvText = (
          <Text style={styles.currentRdvText}>
            Tu as une demande de rendez-vous
          </Text>
        );
      } else {
        currentRdvText = (
          <Text style={styles.currentRdvText}>En attente de la réponse</Text>
        );
      }
    } else {
      currentRdvText = (
        <Text style={styles.currentRdvText}>Tu as un rendez-vous</Text>
      );
    }
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.convKey}>
        {conversation ? (
          ...
          <Text style={styles.textBlocked}>Tu as été bloqué</Text>
        )}
      </View>
      <Animated.View
        ...
        {conversation && (
          <View style={styles.conversationBottom}>
            ...
            <TouchableOpacity
              style={[
                styles.bottomButton,
                Boolean(currentRdv) && { backgroundColor: "#BC8D85" },
              ]}
